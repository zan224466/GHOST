...
class TestPyProfNvtx(unittest.TestCase):
    ...
    def test_conv1d(self):
        ...
        tensor1d_in_conv_grouped = torch.randn(
            32, 6, 224, device="cuda", dtype=self.dtype
        )
        ...
        # conv1d with bias
        conv1d_out_with_bias = F.conv1d(
            tensor1d_in_conv, conv1d_filter, bias=conv1d_bias
        )
        # conv1d - stride > 1
    ...
    def test_conv2d(self):
        ...
        tensor2d_in_conv_grouped = torch.randn(
            32, 6, 224, 224, device="cuda", dtype=self.dtype
        )
    ...
    def test_conv3d(self):
        # Data and weight tensors
        tensor3d_in_conv = torch.randn(
            32, 3, 16, 224, 224, device="cuda", dtype=self.dtype
        )
        tensor3d_in_conv_grouped = torch.randn(
            32, 6, 16, 224, 224, device="cuda", dtype=self.dtype
        )
    ...
    def test_conv_transpose1d(self):
        # Data and weight tensors
        conv_transpose1d_tensor = torch.randn(
            64, 16, 64, device="cuda", dtype=self.dtype
        )
        conv_transpose1d_filter = torch.randn(
            16, 32, 3, device="cuda", dtype=self.dtype
        )
        conv_transpose1d_bias = torch.randn(32, device="cuda", dtype=self.dtype)
        # Conv transpose runs
        conv_transpose1d_out = F.conv_transpose1d(
            conv_transpose1d_tensor, conv_transpose1d_filter
        )
        conv_transpose1d_out_biased = F.conv_transpose1d(
            conv_transpose1d_tensor, conv_transpose1d_filter, bias=conv_transpose1d_bias
        )
        conv_transpose1d_out_strided = F.conv_transpose1d(
            conv_transpose1d_tensor, conv_transpose1d_filter, stride=2
        )
        conv_transpose1d_out_padded = F.conv_transpose1d(
            conv_transpose1d_tensor, conv_transpose1d_filter, padding=3
        )
        conv_transpose1d_out2_padded = F.conv_transpose1d(
            conv_transpose1d_tensor,
            conv_transpose1d_filter,
            output_padding=2,
            dilation=3,
        )
        conv_transpose1d_out_grouped = F.conv_transpose1d(
            conv_transpose1d_tensor, conv_transpose1d_filter, groups=2
        )
        conv_transpose1d_out_dilated = F.conv_transpose1d(
            conv_transpose1d_tensor, conv_transpose1d_filter, dilation=2
        )

    def test_conv_transpose2d(self):
        # Data and weight tensors
        conv_transpose2d_tensor = torch.randn(
            64, 8, 5, 5, device="cuda", dtype=self.dtype
        )
        conv_transpose2d_filter = torch.randn(
            8, 16, 3, 3, device="cuda", dtype=self.dtype
        )
        conv_transpose2d_bias = torch.randn(16, device="cuda", dtype=self.dtype)
        # Conv transpose runs
        conv_transpose2d_out = F.conv_transpose2d(
            conv_transpose2d_tensor, conv_transpose2d_filter
        )
        conv_transpose2d_out_biased = F.conv_transpose2d(
            conv_transpose2d_tensor, conv_transpose2d_filter, bias=conv_transpose2d_bias
        )
        conv_transpose2d_out_strided = F.conv_transpose2d(
            conv_transpose2d_tensor, conv_transpose2d_filter, stride=2
        )
        conv_transpose2d_out_padded = F.conv_transpose2d(
            conv_transpose2d_tensor, conv_transpose2d_filter, padding=3
        )
        ...
        conv_transpose2d_out_grouped = F.conv_transpose2d(
            conv_transpose2d_tensor, conv_transpose2d_filter, groups=2
        )
        conv_transpose2d_out_dilated = F.conv_transpose2d(
            conv_transpose2d_tensor, conv_transpose2d_filter, dilation=2
        )

    def test_conv_transpose3d(self):
        # Data and weight tensors
        conv_transpose3d_tensor = torch.randn(
            20, 16, 50, 10, 20, device="cuda", dtype=self.dtype
        )
        conv_transpose3d_filter = torch.randn(
            16, 33, 3, 3, 3, device="cuda", dtype=self.dtype
        )
        conv_transpose3d_bias = torch.randn(33, device="cuda", dtype=self.dtype)
        # Conv transpose runs
        conv_transpose3d_out = F.conv_transpose3d(
            conv_transpose3d_tensor, conv_transpose3d_filter
        )
    ...
    def test_max_pool1d(self):
        ...
        out = F.max_pool1d(
            inp, kernel_size=5, stride=2, padding=2, return_indices=True, ceil_mode=True
        )
    ...
    def test_rrelu(self):
        inp = torch.randn(1, 3, 32, 32, device="cuda", dtype=self.dtype)
        output = F.rrelu(
            inp, lower=1.0 / 8, upper=1.0 / 3, training=False, inplace=False
        )
    ...
    def test_embedding(self):
        ...
        weight = torch.randn(
            pre_embed_dim, post_embed_dim, device="cuda", dtype=self.dtype
        )
        output = F.embedding(
            inp,
            weight,
    ...
    def test_binary_cross_entropy(self):
        ...
        inp = torch.randn(
            32, 128, device="cuda", dtype=torch.float32, requires_grad=True
        )
        target = torch.randn(
            32, 128, device="cuda", dtype=torch.float32, requires_grad=False
        )
    ...
    def test_poisson_nll_loss(self):
        inp = torch.randn(32, 128, device="cuda", dtype=self.dtype, requires_grad=True)
        target = torch.randn(
            32, 128, device="cuda", dtype=self.dtype, requires_grad=False
        )
    ...
    def test_cosine_embedding_loss(self):
        ...
        output = F.cosine_embedding_loss(
            inp1,
            inp2,
            target,
            margin=0,
    ...
    def test_cross_entropy(self):
        ...
        target = torch.randint(
            0, 100, (32,), device="cuda", dtype=torch.long, requires_grad=False
        )
    ...
    def test_ctc_loss(self):
        ...
        log_probs = (
            torch.randn(50, 16, 20, device="cuda", dtype=torch.float32)
            .log_softmax(2)
            .detach()
            .requires_grad_()
        )
    ...
    def test_hinge_embedding_loss(self):
        ...
        output = F.hinge_embedding_loss(
            inp, target, margin=1.0, size_average=None, reduce=None, reduction="mean"
        )

    def test_kl_div(self):
        inp = torch.randn(32, 128, device="cuda", dtype=self.dtype, requires_grad=True)
        target = torch.randn(
            32, 128, device="cuda", dtype=self.dtype, requires_grad=True
        )
        output = F.kl_div(
            inp, target, size_average=None, reduce=None, reduction="batchmean"
        )
    ...
    def test_mse_loss(self):
        ...
        output = F.mse_loss(
            inp, target, size_average=None, reduce=None, reduction="mean"
        )
    ...
    def test_interpolate(self):
        ...
        output = F.interpolate(
            inp, size=None, scale_factor=2, mode="nearest", align_corners=None
        )
...
def run_tests(precision):
    ...
    test_cases = list(
        filter(
            lambda x: "test_" in x,
            map(lambda x: x[0], inspect.getmembers(dummy, predicate=inspect.ismethod)),
        )
    )
