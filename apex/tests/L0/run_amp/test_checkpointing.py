...
from utils import FLOAT, common_init
...
class TestCheckpointing(unittest.TestCase):
    ...
    def check_state_dict_fp32(self, state_dict):
        for key in state_dict:
            ...
            self.assertEqual(param.type(), FLOAT, "Parameter in state_dict not FLOAT")
                            ...
                            for epoch in range(nb_epochs):
                                ...
                                if epoch == (nb_epochs_restore - 1):
                                    ...
                                    restore_optimizer = optim.SGD(
                                        restore_model.parameters(), lr=self.initial_lr
                                    )

                                    if amp_before_load:
                                        restore_model, restore_optimizer = (
                                            amp.initialize(
                                                restore_model,
                                                restore_optimizer,
                                    ...
                                    restore_optimizer.load_state_dict(
                                    ...
                                    self.assertTrue(
                                        torch.allclose(
                                            output.float(), restore_output.float()
                                        ),
                                        "Output of reference and restored models differ for "
                                        + test_setup,
                                    )
                                    self.compare_models(
                                        model, restore_model, test_setup
                                    )
                        # if opt_level != res_opt_level
                        else:
    ...
    def test_loss_scale_decrease(self):
        ...
        for opt_level in self.test_opt_levels:
            ...
            for factor, update_ls, init_ls in zip(
                nb_decrease_loss_scales, updated_loss_scales, initial_loss_scales
            ):
                self.assertEqual(update_ls, init_ls / 2**factor)

            # Check state dict
            amp_state_dict = amp.state_dict()
            for scaler_idx, factor, init_ls in zip(
                amp_state_dict, nb_decrease_loss_scales, initial_loss_scales
            ):
