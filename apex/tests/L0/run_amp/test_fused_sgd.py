...
class MyModel(torch.nn.Module):
    def __init__(self, unique):
        ...
        self.weight1 = Parameter(
            1.0 + unique + torch.arange(2, device="cuda", dtype=torch.float16)
        )
...
class TestMultipleModelsOptimizersLosses(unittest.TestCase):
    ...
    def test_2models2losses1optimizer(self):
        ...
        optimizer = torch.optim.SGD(
            [
                {"params": model0.parameters(), "lr": 0.25},
                            ...
                            for inject_inf_loc in inject_inf_locs:
                                for which_backward in which_backwards:
                                    ...
                                    for i in range(iters):
                                        ...
                                        with amp.scale_loss(
                                            loss0, optimizer, loss_id=loss_ids[0]
                                        ) as scaled_loss:
                                            scaled_loss.backward()
                                            if i == inject_inf and which_backward == 0:
                                                if inject_inf_loc == "fp32":
                                                    model0.weight0.grad[0] = float(
                                                        "inf"
                                                    )
                                                elif inject_inf_loc == "fp16":
                                                    model0.weight1.grad[0] = float(
                                                        "inf"
                                                    )
                                        with amp.scale_loss(
                                            loss1, optimizer, loss_id=loss_ids[1]
                                        ) as scaled_loss:
                                            scaled_loss.backward()
                                            if i == inject_inf and which_backward == 1:
                                                if inject_inf_loc == "fp32":
                                                    model1.weight0.grad[0] = float(
                                                        "inf"
                                                    )
                                                elif inject_inf_loc == "fp16":
                                                    model1.weight1.grad[0] = float(
                                                        "inf"
                                                    )
                                        ...
                                        if i != inject_inf:
                                            ...
                                            for param, reference_grad in zip(
                                                ...
                                                if (
                                                    opt_level == "O2"
                                                    and not materialize_master_grads
                                                ):
                                                    continue
                                                else:
                                                    self.assertTrue(
                                                        ...
                                                        "opt_level {} i {} inject_inf {} which_backward {} inject_inf_loc {} use_multiple_loss_scalers {}".format(
                                                            opt_level,
                                                            i,
                                                            inject_inf,
                                                            which_backward,
                                                            inject_inf_loc,
                                                            use_multiple_loss_scalers,
                                                        ),
                                                    )
                                            unskipped += 1
                                        optimizer.step()
                                    ...
                                    for model, master, reference in zip(
                                        ...
                                        self.assertTrue(
                                            torch.allclose(model, reference)
                                        )
                                        self.assertTrue(
                                            torch.allclose(
                                                model, master.to(model.dtype)
                                            )
                                        )
                            ...
                            for inject_inf_loc in inject_inf_locs:
                                for which_backward in which_backwards:
                                    ...
                                    for which_model in which_models:
                                        ...
                                        _amp_state.loss_scalers[0]._loss_scale = 4.0
                                        if use_multiple_loss_scalers:
                                            _amp_state.loss_scalers[1]._loss_scale = (
                                                16.0
                                            )
                                        ...
                                        for i in range(iters):
                                            ...
                                            with amp.scale_loss(
                                                ...
                                                if (
                                                    ...
                                                    if inject_inf_loc == "fp32":
                                                        inj_model.weight0.grad[0] = (
                                                            float("inf")
                                                        )
                                                    elif inject_inf_loc == "fp16":
                                                        inj_model.weight1.grad[0] = (
                                                            float("inf")
                                                        )
                                            with amp.scale_loss(
                                                loss1, optimizer, loss_id=loss_ids[1]
                                            ...
                                            if i != inject_inf:
                                                master_params = amp.master_params(
                                                    optimizer
                                                )
                                                for param, reference_grad in zip(
                                                    master_params,
                                                    ...
                                                    if (
                                                        ...
                                                        self.assertTrue(
                                                            ...
                                                            "opt_level {} i {} inject_inf {} which_backward {} inject_inf_loc {} which_model {} use_multiple_loss_scalers {}".format(
                                                                opt_level,
                                                                i,
                                                                inject_inf,
                                                                which_backward,
    ...
    def test_2models2losses2optimizers(self):
        ...
        optimizer0 = torch.optim.SGD(
            [{"params": model0.parameters(), "lr": 0.25}], momentum=0.125
        )
        optimizer1 = torch.optim.SGD(
            [{"params": model1.parameters(), "lr": 0.5}], momentum=0.25
        )
        ...
        for which_iter in (0, 1):
            for which_backward in (0, 1):
                ...
                for i in range(3):
                    ...
                    if i != which_iter:
                        reference_grads[
                            what_got_skipped(which_iter, which_backward)
                        ].append(
                            [param.grad.data.clone() for param in model0.parameters()]
                            + [param.grad.data.clone() for param in model1.parameters()]
                        )
                            ...
                            for inject_inf_loc in inject_inf_locs:
                                for which_backward in which_backwards:
                                    ...
                                    for i in range(iters):
                                        ...
                                        with amp.scale_loss(
                                            loss0, optimizer0, loss_id=loss_ids[0]
                                        ) as scaled_loss:
                                            scaled_loss.backward()
                                            if i == inject_inf and which_backward == 0:
                                                if inject_inf_loc == "fp32":
                                        ...
                                        with amp.scale_loss(
                                            loss1, optimizer1, loss_id=loss_ids[1]
                                        ) as scaled_loss:
                                            scaled_loss.backward()
                                            if i == inject_inf and which_backward == 1:
                                                if inject_inf_loc == "fp32":
                                    ...
                                    master_params = [
                                        p for p in amp.master_params(optimizer0)
                                    ] + [p for p in amp.master_params(optimizer1)]
                                    for model, master, reference in zip(
                                        model_params,
                                        master_params,
                                        final_params[
                                            what_got_skipped(inject_inf, which_backward)
                                        ],
                                    ):
                                        self.assertTrue(
                                            torch.allclose(model, reference)
    ...
    def test_3models2losses2optimizers(self):
        ...
        optimizer1 = torch.optim.SGD(
            [{"params": model2.parameters(), "lr": 0.5}], momentum=0.25
        )
                ...
                for which_model in which_models:
                    ...
                    for i in range(3):
                        ...
                        if i != which_iter:
                            reference_grads[
                                what_got_skipped(
                                    which_iter, which_backward, which_model
                                )
                            ].append(
                                [
                    ...
                    final_params[
                        what_got_skipped(which_iter, which_backward, which_model)
                    ] = (
                        [param.data.clone() for param in model0.parameters()]
                        + [param.data.clone() for param in model1.parameters()]
                        + [param.data.clone() for param in model2.parameters()]
