...
class TestModel(torch.nn.Module):
    def __init__(self, args):
        ...
        self.linear = torch.nn.Sequential(
            *[
                torch.nn.Linear(args.dim, args.dim, bias=args.bias)
...
def main():
    ...
    for i in range(args.steps):
        x_ref = (
            torch.randn(args.batch, args.dim, dtype=torch.half)
            .cuda()
            .requires_grad_(True)
        )
        ...
        for i, (rp, dp) in enumerate(
            zip(ref_model.parameters(), dist_model.parameters())
        ):
