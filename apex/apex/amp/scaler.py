...
def axpby_check_overflow_python(
...
class LossScaler(object):
        ...
        for model, master in zip(model_grads, master_grads):
            if model is not None:
                ...
                self._has_overflow = scale_check_overflow_python(
                    model, master, 1.0 / scale, self.dynamic
                )
    ...
    def unscale_with_stashed_python(
        self, model_grads, stashed_master_grads, master_grads, a, b
    ):
        for model, stashed, master in zip(
            ...
            if model is None and stashed is None:
                ...
                if not LossScaler.warned_unscaling_non_fp32_grad:
                    if master.dtype != torch.float32:
                        ...
                        LossScaler.warned_unscaling_non_fp32_grad = True
                self._has_overflow = axpby_check_overflow_python(
                    model, stashed, master, a, b, self.dynamic
                )
                if self._has_overflow and self.dynamic:
                    break

    def unscale_with_stashed(
        self, model_grads, stashed_master_grads, master_grads, scale_override=None
    ):
        if self._has_overflow:
            return
        ...
        if LossScaler.has_fused_kernel:
            if (
                not LossScaler.warned_unscaling_non_fp32_grad
                and master_grads[0].dtype == torch.float16
            ):
                print(
                    "Warning:  unscaling grads that are not FP32. "
                    "Unscaling non-fp32 grads may indicate an error. "
                    "When using Amp, you don't need to call .half() on your model."
                )
                # Setting this to True unconditionally allows the possibility of an escape
                ...
                LossScaler.warned_unscaling_non_fp32_grad = True
            multi_tensor_applier(
                LossScaler.multi_tensor_axpby_cuda,
                self._overflow_buf,
                [model_grads, stashed_master_grads, master_grads],
            ...
            self.unscale_with_stashed_python(
                model_grads,
