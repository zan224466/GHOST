...
class SyncBatchnormFunction(Function):
    ...
    def forward(
        ...
        if track_running_stats:
            ...
            if torch.distributed.is_initialized():
                ...
                count_t = torch.empty(1, dtype=mean.dtype, device=mean.device).fill_(
                    count
                )
                combined = torch.cat(
                    [mean.view(-1), var_biased.view(-1), count_t], dim=0
                )
            ...
            running_variance.data = (
                running_variance.data * (1 - momentum) + momentum * r_v_inc
            )
        else:
        ...
        ctx.save_for_backward(
            input, weight, mean, inv_std, z, bias, count_all.to(torch.int32)
        )
    ...
    def backward(ctx, grad_output):
        ...
        if fuse_relu:
            grad_output = syncbn.relu_bw_c_last(
                grad_output, saved_input, z, mean, inv_std, weight, bias
            )
