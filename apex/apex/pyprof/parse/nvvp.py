...
class NVVP(object):
    ...
    def getProfileStart(self):
        ...
        for table in [self.driverT, self.runtimeT, self.kernelT, self.markerT]:
            ...
            cmd = "select {} from {} ORDER BY {} ASC LIMIT 1".format(
    ...
    def createMarkerTable(self):
        ...
        cmd = "CREATE TEMPORARY TABLE marker AS SELECT \
					...
					a.id = b.id and \
					a.flags = 2 and b.flags = 4".format(
            self.markerT, self.markerT
        )
        self.db.execute(cmd)
    ...
    def getCPUInfo(self, corrId):
        ...
        cmd = (
            "select start,end,processId,threadId from {} where correlationId={}".format(
                self.runtimeT, corrId
            )
        )
        result = self.db.select(cmd)
        assert len(result) <= 1
        ...
        if len(result) == 0:
            ...
            cmd = "select start,end,processId,threadId from {} where correlationId={}".format(
                self.driverT, corrId
            )
            result = self.db.select(cmd)
    ...
    def getMarkerInfo(self, objId, startTime, endTime):
        ...
        def filterTrace(mlist):
            ...
            mlist = list(
                filter(lambda x: "/torch/autograd/__init__.py" not in x, mlist)
            )
        ...
        cmd = 'SELECT id,name from marker where \
				...
				startTime < {} and \
				endTime > {} \
				ORDER BY startTime ASC'.format(
            objId, startTime, endTime
        )
        result = self.db.select(cmd)
        ...
        for r in result:
            ...
            if (
                ("_backward, seq =" in m)
                or ("Backward, seq =" in m)
                or ("Backward0, seq =" in m)
            ):
                bprop = True
