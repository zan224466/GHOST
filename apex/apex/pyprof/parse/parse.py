...
def parseArgs():
    parser = argparse.ArgumentParser(
        prog=sys.argv[0], description="Parse SQL (nvvp) db."
    )
    parser.add_argument("file", type=str, default=None, help="SQL db (nvvp) file.")
...
def main():
    ...
    if len(kInfo) == 0:
        ...
        print(
            "Found {} kernels. Getting info for each kernel.".format(len(kInfo)),
            file=sys.stderr,
        )
    ...
    for i in tqdm(range(len(kInfo)), ascii=True):
        ...
        if len(k.seqId):
            ...
            if ((currSeqId == prevSeqId) and (k.op == prevOp)) or (
                (k.op[0] == "forward")
                and (k.op == prevOp)
                and (k.mod[0] in ["LSTMCell", "GRUCell", "RNNCell"])
            ):
