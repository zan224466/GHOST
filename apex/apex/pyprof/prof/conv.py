...
class Conv(OperatorLayerBase):
    ...
    def bytes_flops(self):
        ...
        if any(
            ...
            x in self.name
            for x in Conv.convList + Conv.winoList + Conv.fftList + Conv.miscList
        ):
            if g == 1:
                bytes, flops = self.conv_bytes_flops(N, C, H, W, K, P, Q, R, S, g, t)
            else:
                if "2d_grouped_direct_kernel" in self.name:  # only 1 kernel is called
                    bytes, flops = self.conv_bytes_flops(
                        N, C, H, W, K, P, Q, R, S, g, t
                    )
                elif (
                    "spatialDepthwiseConvolutionUpdateOutput" in self.name
