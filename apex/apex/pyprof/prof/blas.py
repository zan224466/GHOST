...
class Addmm(OperatorLayerBase):
    ...
    def params(self):
        p = OrderedDict(
            [("M", self.n), ("N", self.m), ("K", self.k), ("type", self.type)]
        )
        return p
...
class Matmul(OperatorLayerBase):
    ...
    def __init__(self, d):
        ...
        assert ((mod == "torch") and (op == "matmul")) or (
            (mod == "Tensor") and (op == "__matmul__")
        )
        assert len(args) == 2
    ...
    def bytes(self):
        ...
        if self.name in Matmul.NON_GEMM:
            return (
                2 * Utility.typeToBytes(self.type) * Utility.numElems(self.A)
            )  # could be B as well
        else:
            m, n, k = self.m, self.n, self.k
            return Utility.typeToBytes(self.type) * (m * n + m * k + n * k)
