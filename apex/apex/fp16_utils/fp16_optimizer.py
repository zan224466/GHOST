    ...
    def _step_with_closure(self, closure):
        def wrapped_closure():
            ...
            while self.overflow:
                ...
                print(
                    "OVERFLOW within closure! Skipping step, reducing loss scale to {}".format(
    ...
    def update_master_grads(self):
        ...
        if len(self.all_fp16_params) > 0:
            ...
            for model_param, master_param in zip(
                self.all_fp16_params, self.all_fp32_from_fp16_params
            ):
            ...
            self.loss_scaler.unscale(
                model_grads, master_grads, self.loss_scaler.loss_scale()
            )
        ...
        if len(self.all_fp32_from_fp32_params) > 0:
            ...
            master_grads = []
            for model_param, master_param in zip(
                self.all_fp32_from_fp32_params, self.all_fp32_from_fp32_params
            ):
                if model_param.grad is not None:
                    model_grads.append(model_param.grad)
                    master_grads.append(master_param.grad)
