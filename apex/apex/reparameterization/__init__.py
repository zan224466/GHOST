...
def apply_weight_norm(module, name="", dim=0, hook_child=True):
    ...
    return apply_reparameterization(
        module, reparameterization=WeightNorm, hook_child=hook_child, name=name, dim=dim
    )
...
def remove_weight_norm(module, name="", remove_all=False):
    ...
    return remove_reparameterization(
        module, reparameterization=WeightNorm, name=name, remove_all=remove_all
    )


def apply_reparameterization(
    module, reparameterization=None, name="", dim=0, hook_child=True
):
    """
...
def remove_reparameterization(
    module, reparameterization=Reparameterization, name="", remove_all=False
):
    """
    ...
    if name != "" or remove_all:
        ...
        for k, hook in module._forward_pre_hooks.items():
            if isinstance(hook, reparameterization) and (
                hook.name == name or remove_all
            ):
                hook.remove(module)
                to_remove.append(k)
        if len(to_remove) > 0:
        ...
        if not remove_all:
            raise ValueError(
                "reparameterization of '{}' not found in {}".format(name, module)
            )
    else:
        modules = [module] + [x for x in module.modules()]
        for m in modules:
            remove_reparameterization(
                m, reparameterization=reparameterization, remove_all=True
            )
        return module
