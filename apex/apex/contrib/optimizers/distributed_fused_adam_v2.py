...
class DistributedFusedAdamV2(torch.optim.Optimizer):
    ...
    def __init__(
        ...
        def _flat_split(p):
            ...
            def __chunkify(p):
                return [
                    ...
                    for chunk_id in range(self._group_size)
                ]
            ...
            list_of_list_of_list_of_chunks = [
                [__chunkify(shard) for shard in shards]
                for shards in list_of_list_of_shards
            ]
            return (
                list_of_blocks,
        ...
        self._flat_grads_blocks, self._flat_grads_shards, self._flat_grads_chunks = (
        ...
        for shard_id in range(self._group_size):
            for block_id in range(self._num_blocks):
                flat_shard_start = (
                    block_id * self._group_size + shard_id
                ) * self._shard_size
                ...
                for p, grads_info in zip(self._model_params, self._grads_info):
                    ...
                    if clipped_start < clipped_end:
                        ...
                        new_param_packed_fragment = self._new_params_mega_blocks[
                            shard_id
                        ][block_id][shard_offset : shard_offset + grad_length]
                        ...
                        if shard_id == self._rank_in_group:
                            ...
                            master_param_fragment = self._fp32_p_blocks[block_id][
                            ...
                            print(
                                ...
                                % (
                                    str(model_param_fragment.size()),
                                    str(new_param_packed_fragment.size()),
                                    str(master_param_fragment.size()),
                                )
                            )
        ...
        self._distributed_weight_update = (
            distributed_weight_update  # Is this still needed?
        )
    ...
    def _pipeline_block_reductions(self, block_id):
        ...
        with torch.cuda.stream(rs_stream):
            rs_work = torch.distributed.reduce_scatter(
                self._fp16_g_blocks[block_id],
                self._flat_grads_shards[block_id],
                group=self._rs_pg[block_id % self._num_rs_pg],
                async_op=True,
                no_copy=True,
            )
            for chunk_id in range(self._num_chunks):
        ...
        with torch.cuda.stream(self._completion_st):
            for block_id in range(self._num_blocks):
                for chunk_id in range(self._num_chunks):
                    self._reductions_works[block_id][chunk_id].wait()
            self.__launch_step_kernel(
                self._fp32_p, self._fp16_p, self._fp32_m, self._fp32_v, self._fp16_g
            )
            torch.distributed.all_gather(
                self._new_params_mega_shards,
                self._fp16_p,
    ...
    def step(self, closure=None, skip_overflow_check=False):
        ...
        with torch.cuda.stream(self._completion_st):
            ...
            has_overflow = (
                ...
                else self.strided_check_finite(
                    self._new_params,
