...
class DistributedFusedAdam(torch.optim.Optimizer):
    ...
    def __init__(
        ...
        self._current_process_group = (
            current_process_group
            if current_process_group is not None
            else c10d._get_default_group()
        )
        self._available_ranks = list(
            c10d._pg_group_ranks[self._current_process_group].keys()
        )
        self._process_group_id = process_group_id
        self._process_group_size = (
            torch.cuda.device_count() if process_group_size <= 0 else process_group_size
        )
    ...
    def _first_step_init(self):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                torch.distributed.broadcast(
                    p, src=self._available_ranks[0], group=self._current_process_group
                )
                ...
                self._group_properties.append(
                    (beta1, beta2, bias_correction, eps, weight_decay)
                )
                ...
                def wrapper(param, param_i, param_grads_size, param_offset):
                    ...
                    def allreduce_hook(*unused):
                        self._do_overlapped_reduction(
                            param_i, param_grads_size, param_offset, param
                        )
        ...
        for p_i, (grads_info, p) in enumerate(
            zip(self._grads_info, self._model_params)
        ):
            self._individual_flat_grads.append(
                self._flat_grads[
                    grads_info["param_offset"] : grads_info["param_offset"]
                    + grads_info["param_grads_size"]
                ].view_as(p)
            )
        ...
        self._flat_grads_blocks, self._flat_grads_chunks, self._flat_grads_shards = (
        ...
        def _full_packed_split(p):
            def __shardify(p):
                return [
                    p[
                        mega_shard
                        * self._mega_shard_size : (mega_shard + 1)
                        * self._mega_shard_size
                    ]
        ...
        for shard_id in range(self._group_size):
            for block_id in range(self._num_blocks):
                for chunk_id in range(self._num_chunks):
                    ...
                    for p, grads_info, group_props in zip(
                        ...
                        if clipped_start < clipped_end:
                            ...
                            self._packed_flat_to_model_params.append(
                            ...
                            if shard_id == self._group_rank:
                                ...
                                if (
                                    self._model_parallel
                                    and hasattr(p, "model_parallel")
                                    and not p.model_parallel
                                ):
                                    self._non_parallel_grads.append(
                                        opti_state_g_fragment
                                    )
        ...
        beta1, beta2, bias_correction, epsilon, decay = list(
        ...
        if self._num_groups > 1:
            ...
            for i in range(self._num_process_groups):
                ...
                ranks = [
                    i + k * self._num_process_groups
                    for k in range(self._process_group_size)
                ]
                ...
                if self._compute_L2_grad_norm:
                    ...
                    if self._global_rank in rs_rank:
                        ...
                        torch.distributed.all_reduce(
                            self._overflow_buf, group=self._l2_grad_norm_pg
                        )
        ...
        self._l2_grad_norm_st = (
            torch.cuda.Stream() if self._compute_L2_grad_norm else None
        )
    ...
    def _pipeline_block_reductions(self, block_id):
        ...
        if self._compute_L2_grad_norm and block_id == 0:
            with torch.cuda.stream(self._l2_grad_norm_st):
                ...
                if (
                    self._model_parallel and self._process_group_id
                    ...
                    torch.distributed.all_reduce(
                        non_parallel_grad_norm_sq, group=self._l2_grad_norm_pg
                    )
    ...
    def __launch_step_kernel(self):
        ...
        if (
            ...
            and self._param_group["max_grad_norm"] > 0
            and math.isfinite(self.L2_grad_norm)
        ):
            combined_scale = self._param_group["max_grad_norm"] / (
                self.L2_grad_norm / self._global_scale + 1e-6
            )
    ...
    def _do_overlapped_reduction(self, param_i, param_grads_size, param_offset, param):
        ...
        if self._flat_mt:
            ...
            torch.div(
                param.grad,
                self._world_size if self._predivide else 1.0,
                out=self._individual_flat_grads[param_i],
            )
    ...
    def strided_check_finite(
        self, output_params, stride=1, start=-1, end=-1, clear=True
    ):
        ...
        if start >= 0 and start < end:
            ...
            out_p = output_params
        fused_adam_cuda.strided_check_finite(
            self._overflow_buf, out_p, stride, 1 if clear else 0
        )
