...
class DistributedFusedAdamV3(torch.optim.Optimizer):
    ...
    def __init__(
        ...
        def _flat_split(flat):
            def __flat_blockify(flat):
                return [
                    flat[
                        block_id * self._block_size : (block_id + 1) * self._block_size
                    ]
            ...
            def __flat_shardify(flat):
                return [
                    flat[
                        shard_id * self._shard_size : (shard_id + 1) * self._shard_size
                    ]
        ...
        self._flat_params_blocks, self._flat_params_shards = _flat_split(
            self._flat_params
        )

        # master params
        self._fp32_p = torch.zeros(
            [self._shard_size], dtype=torch.float32, device="cuda"
        )
        ...
        for group_i in range(self._num_groups):
            ranks = [
                group_i * self._group_size + local_rank
                for local_rank in range(self._group_size)
            ]
    ...
    def has_overflow(self):
        return (
            True
            if not self.L2_grad_norm is None and not math.isfinite(self.L2_grad_norm)
            else False
        )
        ...
        if not self._last_step and self._overlap_reductions:
            ...
            while flush_block:
                ...
                with torch.cuda.stream(self._dwu_st):
                    self._flatten_grad_mt(
                        1.0 / self._world_size if self._predivide else 1.0
                    )
