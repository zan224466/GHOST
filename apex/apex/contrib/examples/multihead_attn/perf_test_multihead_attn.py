...
from apex.contrib.multihead_attn import EncdecMultiheadAttn, SelfMultiheadAttn

parser = argparse.ArgumentParser(description="Multihead Attention Standalone Test")
parser.add_argument(
    "--seq-length", default=64, type=int, help="Sequence Length of Input"
)
parser.add_argument(
    "--num-seqs-start", default=10, type=int, help="Start Range of Number of Sequences"
)
parser.add_argument(
    "--num-seqs-stop", default=120, type=int, help="Stop Range of Number of Sequences"
)
parser.add_argument(
    "--num-seqs-inc", default=5, type=int, help="Range Increment of Number of Sequences"
)
parser.add_argument(
    "--trials", default=20, type=int, help="Number of Trials to Execute"
)
parser.add_argument(
...
parser.add_argument(
    "--hidden-dim", default=1024, type=int, help="Multihead Attention hidden dimension"
)
parser.add_argument(
    "--heads", default=16, type=int, help="Number of Multihead Attention heads"
)
parser.add_argument(
    "--encdec-attn",
    action="store_true",
    help="Use Encoder-Decoder Attention instead of Self Attention.",
)
parser.add_argument(
    "--norm-add",
    action="store_true",
    help="Include Layer Norm and Dropout-Add in Multihead Attention block.",
)
parser.add_argument(
    "--ref", action="store_true", help="Reference implementation in python pytorch."
)
parser.add_argument(
    "--native", action="store_true", help="torch.nn.MultitheadAttention Version."
)
parser.add_argument("--fwd", action="store_true", help="Only execute Fwd Pass.")
parser.add_argument(
    "--biases",
    action="store_true",
    help="Execute multihead attention with Linear Biases.",
)
        ...
        if args.native:
            attn_layers.append(
                torch.nn.MultiheadAttention(
                    args.hidden_dim, args.heads, dropout=0.1, bias=args.biases
                )
            )
        elif args.ref:
            attn_layers.append(
...
for sequences in range(
    args.num_seqs_start, args.num_seqs_stop + args.num_seqs_inc, args.num_seqs_inc
):
    inputs = torch.randn(
        args.seq_length,
        sequences,
    ...
    for trial in range(0, args.trials + args.warmup_trials):
        ...
        for lyr_idx in range(0, args.layers):
            if args.native:
                outputs, _ = attn_layers[lyr_idx].forward(
                    layer_inputs,
                    layer_inputs,
                    layer_inputs,
    ...
    print(
        "[ {} Attn {} ]Total Tokens: {:4d} Sequences: {:3d} Sequence Length: {:3d} Fwd Time / Layer: {:.3f} ms Bwd Time / Layer: {:.3f} ms".format(
